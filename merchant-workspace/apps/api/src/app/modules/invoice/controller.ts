...
// router
invoiceRouter.get('/:id(\\d+)', asyncHandler(getInvoice.handler));
invoiceRouter.get('/public/:id(\\d+)', asyncHandler(getPublicInvoice.handler));
invoiceRouter.get('/list', asyncHandler(getInvoiceList.handler));
invoiceRouter.get('/invoice-position/:id(\\d+)', getInvoicePosition.handler);
...
eventService.onCommandHandler<
  CommandInvoicePositionCreate & { type: 'command' }
>('command.invoicePosition.create', createInvoicePosition.handler);
eventService.onCommandHandler<
  CommandInvoicePositionUpdate & { type: 'command' }
>('command.invoicePosition.update', updateInvoicePosition.handler);
eventService.onCommandHandler<
  CommandInvoicePositionDelete & { type: 'command' }
>('command.invoicePosition.delete', deleteInvoicePosition.handler);
