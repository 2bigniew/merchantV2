...
export type WhereSource = string | number | boolean | Array<string> | Array<number> | undefined | null | WhereDate;
...
export const prepareConditions = (obj: Record<string, WhereSource>, separator: WhereSeparator = "AND"): string => {
  ...
  for (const key in dbObj) {
    ...
    if (typeof dbObj[key] === "object" && dbObj[key] instanceof Array) {
      ...
      where.push({
        field: key,
        condition: "IN",
        value: `(${values.map((value) => (typeof value === "string" ? `'${value}'` : value)).join(", ")})`
      });
    }
...
export const removeUndefined = <T>(arr: Array<T | undefined>): T[] => arr.filter((el) => el !== undefined) as T[];
